...
// import { AddressValidator, AmlDataTypeValidator, BankAccountValidator, BasicPhoneValidator, BodyValidator, CACValidator, DriversValidator, EcosystemValidator, KarmaValidator, RadarBvnValidator, RadarPhoneValidator, createBvnidentity, createCRCidentity, createCreditPremiumidentity, createFirstCreditidentity, createNinidentity, createPhoneidentity, createVotersCardidentity, options } from "../../utils/utils";
...
//     const endpoint = `/identity-verification?data_type=${data_type}`;
...
export const verification = async (
  ...
  try {
    ...
    // const validateBody = BodyValidator.validate(req.body);
    // if (validateBody.error) {
    //   return res
    //     .status(400)
    //     .json({ error: validateBody.error.details[0].message });
    // }
    console.log("I got here",req.body.data)
    // const existingAddress = await AddressIdentityInstance.findOne({
    //   where: {
    //     addressInstanceId: req.body.data,
    //   },
    // });
  ...
  //     const savedResponse = await AddressIdentityInstance.create({
  //       id: createnewAddress.idempotency_ref,
  //       addressInstanceId: createnewAddress.data, 
  //       data: response.data,
  //     });
  //     return res
  //     .status(201)
  ...
  switch (data.data_type) {
    ...
    case 'voters_card':
      return {
        idempotency_ref:uuidv4(),
        data_type: data.data_type,
        voters_card:data.data
        };
    case 'credit_crc':
        return {
        idempotency_ref:uuidv4(),
        data_type: data.data_type,
        credit_crc:data.data
        };
    case 'credit_first_central':
      ...
      return {
        idempotency_ref:uuidv4(),
        data_type: data.data_type,
        credit_first_central:data.data
        };
    case 'credit_premium':
      return {
        idempotency_ref:uuidv4(),
        data_type: data.data_type,
        credit_premium:data.data
        };
    case 'aml':
      return {
        idempotency_ref:uuidv4(),
